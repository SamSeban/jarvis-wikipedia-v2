...
if [[ "$jv_pg_wk_definition" =~ "peut désigner :" || $(echo "$jv_pg_wk_result" | jq -r '.[2][0]') < 20 || "$jv_pg_wk_definition" =~ "peut faire référence à :" || "$jv_pg_wk_definition" =~ "désigne notamment :" ]]
then
